...
class Supplier_orders_model extends CI_Model {


	 var $table = 'supplier_orders';
    var $primary_key_field = 'id';
    var $column = array('','id','user_id','owner_name','owner_address','owner_gender','owner_email','owner_phone','owner_aadhaar','owner_licence'); //set column field database for order and search
    ...
    {
        $this->db->select("m.*, sm.*,m.name as product_name, c.name AS `city_name`, concat('".base_url()."uploads/images/',m.image_url) as image" );
    ...
    {
        $this->db->select($this->table.'.*, cities.name');
        $this->db->from($this->table);
        $this->db->join('cities', 'cities.id=machine_city_id', 'LEFT');
        ...
        foreach ($this->column as $item) // loop column
        {
            ...
            $temp = explode(".",$item);
            $item = (isset($temp[1]))?$item: $this->table.".".$item;
